...
namespace BLL.Service
{
	public class ExamUploadService : IExamUploadService
	{
		...
		public ExamUploadService(IUnitOfWork unitOfWork, IConfiguration configuration, IDriveService driveService)
		{
		...
		public async Task<long> InitiateUploadAsync(IFormFile zipFile, long examId)
		{
			...
			// Get or create Uploads folder inside Exam folder
			var (uploadsFolderId, _) = await _driveService.GetOrCreateFolderAsync("Uploads", exam.DriveFolderId);

			// Upload ZIP file to Drive
			string driveZipFileId;
			string driveZipWebViewLink;
			using (var zipFileStream = File.OpenRead(zipFilePath))
			{
				var (fileId, webViewLink) = await _driveService.UploadFileAsync(zipFileStream, fileName, uploadsFolderId);
				driveZipFileId = fileId;
				driveZipWebViewLink = webViewLink;
			}
			...
			var examZip = new ExamZip
			{
				...
				ZipPath = zipFilePath, // Store ZIP file path (temporary, will be deleted after processing)
				DriveZipFileId = driveZipFileId,
				DriveZipWebViewLink = driveZipWebViewLink,
